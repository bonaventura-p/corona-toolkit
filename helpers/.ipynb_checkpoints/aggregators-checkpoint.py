...
def daily_overview ():
    ...
    data = getJsonData(days_ago=7) #all patients

    path = "/tmp/"

    for patient in data['data']:
        html_out, outfile = __corona_report__(data=patient, keys=coronaDict)
...
def __corona_report__(data: Dict[str, Any], keys: Dict[Any, Any], appname: str = 'tech.syncvr.fysio', lang: str = 'dutch') -> str:
    '''add description'''
...
def convert_to_pdf(body_html: str, outfile: str):
    return HTML(string=body_html).write_pdf(outfile, stylesheets=["helpers/style.css"])
